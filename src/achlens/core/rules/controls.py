    ...
    for batch, control in _batches(context):
        ...
        if actual != expected:
            yield _emit(
                ...
                expected,
                actual,
                "Batch control number does not match the header.",
            )
...
def fc001(context: ValidationContext) -> Iterable[Finding]:
    ...
    if _int(actual) != expected:
        yield _emit(
            ...
            expected,
            actual,
            "File batch count does not match.",
        )
...
def fc002(context: ValidationContext) -> Iterable[Finding]:
    ...
    if _int(actual) != expected:
        yield _emit(
            ...
            expected,
            actual,
            "File block count does not match.",
        )
...
def fc003(context: ValidationContext) -> Iterable[Finding]:
    ...
    if _int(actual) != expected:
        yield _emit(
            ...
            expected,
            actual,
            "File entry/addenda count does not match.",
        )
...
def fc004(context: ValidationContext) -> Iterable[Finding]:
    ...
    if _int(actual) != expected:
        yield _emit(
            ...
            expected,
            actual,
            "File entry hash does not match.",
        )
...
_RULES: dict[str, Rule] = {
    f"BC{index:03d}": globals()[f"bc{index:03d}"] for index in range(1, 10)
}
_RULES.update({f"FC{index:03d}": globals()[f"fc{index:03d}"] for index in range(1, 8)})
control_rule_registry = RuleRegistry(
    spec
    for spec in default_rule_registry().specs.values()
    if spec.category in {"BC", "FC"}
)
...
def validate_controls(context: ValidationContext | str) -> list[Finding]:
    ...
    return [
        finding
        for rule in control_rule_registry.implementations.values()

...
def _context(
    sec: str, addenda: list[Record], trace: str = "123456780000001"
) -> ValidationContext:
    header = Record(
        2,
        "5",
    ...
    return ValidationContext(
        "", None, AchFile(batches=[Batch(header, [Entry(detail, addenda)])])
...
def test_registry_has_exactly_all_addenda_rules() -> None:
    ...
    assert set(addenda_rule_registry.implementations) == set(
        addenda_rule_registry.specs
    )
...
def test_valid_ppd_ccd_web_addenda_and_parent_trace_linkage() -> None:
    ...
    assert "AD004" in _ids(
        _context("PPD", [_05(entry_detail_sequence_number="7654321")])
    )
...
def test_ad001_allows_only_context_specific_addenda_types() -> None:
    for sec, valid in (
        ...
        assert "AD001" not in _ids(
            _context(sec, [_record("addenda", 4, {"addenda_type_code": valid}, "7")])
        )
...
def test_return_addenda_recognizes_codes_and_requires_fifteen_digit_trace() -> None:
    ...
    findings = validate_addenda(
        _context(
            "RET", [_99(return_reason_code="R99", original_entry_trace_number="123")]
        )
    )
...
def test_noc_addenda_recognizes_codes_and_requires_corrected_data() -> None:
    ...
    findings = validate_addenda(
        _context("NOC", [_98(change_code="C99", corrected_data="   ")])
    )
...
def test_malformed_widths_and_findings_include_line_and_field_position() -> None:
    findings = validate_addenda(
        _context(
            "PPD",
            [_05(addenda_sequence_number="001", entry_detail_sequence_number="1")],
        )
    )
    assert {finding.rule_id for finding in findings} == {"AD003", "AD004"}
    assert all(
        finding.line_number == 4 and finding.position is not None
        for finding in findings
    )

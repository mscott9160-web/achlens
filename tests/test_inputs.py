...
def test_content_input_and_exactly_one_input_contract() -> None:
    ...
    assert (
        _error(resolve_input, content="a", path="file.ach").error.code
        == "INPUT_CONFLICT"
    )
...
def test_content_size_limit_is_checked_in_bytes() -> None:
    ...
    assert (
        _error(resolve_input, content="abcd", config=config).error.code == "TOO_LARGE"
    )
...
def test_allowlisted_path_is_read_and_size_checked(tmp_path: Path) -> None:
    ...
    assert (
        _error(resolve_input, path=str(file_path), config=too_small).error.code
        == "TOO_LARGE"
    )
...
def test_path_traversal_and_missing_paths_are_rejected(tmp_path: Path) -> None:
    ...
    assert (
        _error(resolve_input, path=str(outside), config=config).error.code
        == "PATH_NOT_ALLOWED"
    )
    assert (
        _error(resolve_input, path=str(root / "missing.ach"), config=config).error.code
        == "PATH_NOT_FOUND"
    )
...
def test_symlink_escape_is_rejected_when_supported(tmp_path: Path) -> None:
    ...
    config = ServerConfig(allowed_roots=(root.resolve(),))
    assert (
        _error(resolve_input, path=str(link), config=config).error.code
        == "PATH_NOT_ALLOWED"
    )
...
def test_directories_and_non_utf8_files_are_rejected(tmp_path: Path) -> None:
    ...
    assert (
        _error(resolve_input, path=str(tmp_path), config=config).error.code
        == "PATH_NOT_ALLOWED"
    )
    binary = tmp_path / "binary.ach"
    binary.write_bytes(b"\xff")
    assert (
        _error(resolve_input, path=str(binary), config=config).error.code
        == "UNSUPPORTED"
    )

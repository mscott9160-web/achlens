...
def _parsed_file() -> object:
    file_header = _record(
        "file_header", record_type_code=1, priority_code=1,
        immediate_destination="123456789", immediate_origin="987654321",
        file_creation_date=260911, file_id_modifier="A", record_size=94,
        blocking_factor=10, format_code=1,
    )
    batch_header = _record(
        "batch_header", record_type_code=5, service_class_code=200,
        company_name="Company", company_identification="1234567890",
        standard_entry_class_code="WEB", company_entry_description="PAYROLL",
        effective_entry_date=260911, originator_status_code=1,
        originating_dfi_identification=12345678, batch_number=1,
    )
    entry = _record(
        "entry_detail_web", record_type_code=6, transaction_code=22,
        receiving_dfi_identification=12345678, check_digit=0,
        dfi_account_number="ACCOUNT123456789", amount=1250,
        individual_identification_number="ID-12345678901", individual_name="Jane Doe",
        addenda_record_indicator=1, trace_number=123456789,
    )
    noc = _record(
        "addenda_98_noc", record_type_code=7, addenda_type_code=98,
        change_code="C01", original_entry_trace_number=123456789012345,
        original_receiving_dfi_identification=12345678,
        corrected_data="CORRECTED-ACCOUNT-123456789", trace_number=123456789012345,
    )
...
def test_masks_sensitive_fields_across_nested_records() -> None:
    ...
    assert entry.detail.fields["individual_identification_number"].value == "**********8901"
    assert entry.addenda[0].fields["corrected_data"].value == "***********************6789"
...
def test_reveal_is_explicit_and_source_is_unchanged() -> None:
    ...
    assert revealed.batches[0].entries[0].detail.fields["dfi_account_number"].value == "ACCOUNT123456789"
